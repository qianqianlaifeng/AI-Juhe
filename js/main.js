(function() {
  ...
  const DEFAULT_TOOLS = [
    ...
    },
    {
      id: 'tool_videoshot',
      title: '视频截图提取',
      url: '视频截图提取/index.html',
      description: '上传视频，逐帧精确定位或按间隔／张数批量提取画面截图，支持 PNG / JPEG / WebP 与输出尺寸调整，多张可一键打包 ZIP 下载，全程浏览器本地处理、视频不上传。',
      tags: ['视频处理', '截图', '批量导出', '本地'],
      color: 'blue',
      image: '',
      badge: '🛠️ 自研',
      badgeClass: 'badge-self',
    ...
    {
      id: 'tool_promptworkshop',
      title: 'AI 提示词工坊',
      url: 'AI提示词工坊/index.html',
      description: '实时浏览社区 AI 图片与视频作品，点开即可查看完整正向 / 负向提示词与生成参数（模型 / 采样器 / 步数 / CFG / 种子）。提示词在浏览器本地解析、不上传，支持关键词搜索、模型与来源筛选、一键复制全部、看原图。',
      tags: ['提示词', '社区图库', '实时'],
      color: 'cyan',
      image: '',
      badge: '🛠️ 自研',
    ...
    },
    {
      id: 'tool_layer_split',
      title: 'AI 图层分离',
      url: 'AI图层分离/index.html',
      description: '上传一张图，本地 AI 自动把画面里每个元素各自拆成一层透明图层，也能「点哪拆哪」单独抠出指定元素，并导出 PSD 分层文件到 Photoshop / Photopea 继续编辑。自带改图编辑器：移动 / 缩放 / 旋转 / 调色 / 换背景 / 加文字 / 裁剪，导出 PNG·JPG·PSD；另有免费高清放大。全程浏览器本地处理、图片不上传。',
      tags: ['图层分离', '改图编辑', 'PSD导出', '高清放大', '本地'],
      color: 'purple',
      image: '',
      badge: '🛠️ 自研',
      badgeClass: 'badge-self',
  ...
  function initAihot() {
    ...
    async function loadFromSources() {
      const sources = [
        // 专用 AI 实时资讯源（https://60s.viki.moe，ACAO:*，无需中转）
        { name:'ai-news', url:'https://60s.viki.moe/v2/ai-news', map: d => {
          const arr = (d && d.data && Array.isArray(d.data.news)) ? d.data.news
                    : (d && Array.isArray(d.data)) ? d.data
                    : pickArray(d);
          return arr.map(x => ({ title: x.title, url: x.link || x.url || '', hot: String(x.hot || x.hotValue || '') }));
        } },
        // 知乎热榜（codelife 镜像，已验证可跨域，作为补充源）
        { name:'zhihu-codelife', url:'https://api.codelife.cc/api/top/list?lang=cn&id=mproPpoq6O&size=50', map: d => {
